...
public class WorldMap {
    ...
    WorldMap(int mapWidth, int mapHeight, int mapDepth, Snake snake) {
        this(mapWidth, mapHeight, mapDepth, snake, 30);
    }

    private WorldMap(int mapWidth, int mapHeight, int mapDepth, Snake snake, int appleEnergy) {
    ...
    private boolean isOccupied(Vector3D field) {
    ...
    boolean isInsideMap(Vector3D pos) {
    ...
    boolean placeAppleAtRandomCell() {
        if(apple != null) {
            throw new GameLogicException("Apple already placed");
        }
